...
// Verificar si el usuario logueado es un proponente o DAF
$usuario_rol = $_SESSION['rol'];  // Verificar el rol del usuario logueado

// Si el usuario no tiene rol "proponente" o "DAF", no debe ver el proyecto
if ($usuario_rol !== 'proponente' && $usuario_rol !== 'DAF') {
...
$sql = "SELECT 
        ...
        WHERE p.id_proyecto = ?";  // Eliminamos la restricción del estado
...
// Ahora solo estamos pasando el id_proyecto y el id_usuario
$stmt->bind_param("i", $id_proyecto); // Solo enlazamos el id_proyecto

$stmt->execute();
$result = $stmt->get_result();

// Verificar si encontramos el proyecto
if ($result->num_rows === 0) {
    die("No se encontró el proyecto con ID: $id_proyecto");
}

$data = $result->fetch_assoc();  // Asignamos los datos del proyecto
$stmt->close();
$conn->close();